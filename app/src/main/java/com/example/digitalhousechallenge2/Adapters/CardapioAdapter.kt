...
class CardapioAdapter(
       private val cardapioList : List<Pratos>
    ...
    override fun onBindViewHolder(holder: CardapioViewHolder, position: Int) {
        ...
        Glide.with(holder.itemView.context).load(cardapioList[position].imgGrandePrato).into(holder.imageView2)
    ...
    class CardapioViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
         ...
         // preDescricaoItem.text = cardapio.pratoDescricao


      }
    }

// holder.bind(cardapioList[position],onItemClicked)
//    fun bind(cardapio: Pratos, onItemClicked: (Int) -> Unit) = with(itemView) {
//        Glide.with(itemView.context).load(cardapio.imagePrato).into(cardapioItem)
//        Glide.with(itemView.context).load(cardapio.imgGrandePrato).into(ivCardapioMenu)


//        mMcontainer.setOnClickListener{
//            onItemClicked(this@CardapioViewHolder.adapterPosition)
//        }
//
//